...
function DataTypesDiag() {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 w-full max-w-3xl px-4">
      {types.map((t, i) => (
        <motion.div key={i} initial={{ opacity: 0, scale: 0.85 }} whileInView={{ opacity: 1, scale: 1 }}
          transition={{ delay: i * 0.08 }} className={`p-3 sm:p-4 rounded-2xl border-2 text-center ${t.color}`}>
          <code className="font-bold text-lg sm:text-xl block">{t.type}</code>
          <div className="text-xs sm:text-sm font-semibold mt-1">{t.bytes}</div>
          <div className="text-[10px] sm:text-xs opacity-60 mt-1">{t.range}</div>
        </motion.div>
      ))}
    </div>
  );
...
function ArraysDiag() {
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-2xl px-4">
      <code className="text-sm sm:text-lg font-mono text-slate-700 break-all">int scores[5] = {'{85, 92, 78, 96, 88}'};</code>
      <div className="flex flex-wrap justify-center gap-2">
        {cells.map((val, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.1 }} className="flex flex-col items-center">
            <div className="w-10 h-10 sm:w-14 sm:h-14 bg-blue-100 border-2 border-blue-400 rounded-xl flex items-center justify-center font-bold text-blue-900 text-sm sm:text-lg font-mono">{val}</div>
            <div className="text-[10px] sm:text-xs text-slate-400 mt-1 font-mono">[{i}]</div>
          </motion.div>
        ))}
      </div>
      <div className="text-[10px] sm:text-sm text-slate-500">← Index 0 to length−1 →</div>
    </div>
  );
}
...
function FunctionsDiag() {
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-2xl px-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 w-full text-center">
        {[
          ...
          <div key={i} className={`p-3 sm:p-4 rounded-2xl border-2 ${item.color}`}>
            <code className="font-bold text-sm sm:text-base block">{item.ex}</code>
            <div className="text-[10px] sm:text-xs mt-1 opacity-70">{item.label}</div>
          </div>
        ))}
      </div>
